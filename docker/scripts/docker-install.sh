...
# Author:Derek King, adapted from original work, Roberto Rodriguez (@Cyb3rWard0g)
# Feb 2022. 
# License: GPL-3.0
...
install_docker(){
    ...
    if [ $ERROR -ne 0 ]; then
        ...
        if [ -x "$(command -v snap)" ]; then
            ...
            echo "$DOCKER_INFO_TAG Docker could not be installed. Check /var/log/docker-install.log for details."
            exit 1
        fi
    fi
}
